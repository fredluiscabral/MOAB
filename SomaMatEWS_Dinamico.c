...
void somarMatrizes(double* matriz1, double* matriz2, double* resultado, int N, int thread_id, int num_threads) {
    ...
    for (int i = inicio; i <= fim; i++) {
        for (int j = 0; j < N ; j++) {
            resultado[j*N+i] = matriz1[j*N+i] + matriz2[j*N+i];
        }
    }
}
...
*/
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            matriz1[j*N+i] = 1; //rand() % 100;
            matriz2[j*N+i] = 2; //rand() % 100;
        }
    }
