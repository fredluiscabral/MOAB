...
void setValue (double matriz[TAM][TAM], int i, int j, double value) {
        
        matriz[i][j] = value;

}

double getValue (double matriz[TAM][TAM], int i, int j){
...
void somarMatrizes(double matriz1[TAM][TAM], double matriz2[TAM][TAM], double resultado[TAM][TAM], int N, int thread_id, int num_threads, int TILE_SIZE) {
...
*/
    ...
    for (int i = inicio; i < fim; i += TILE_SIZE) {
            for (int j = 0; j < N; j += TILE_SIZE) {
                for (int ti = i; ti < min(i + TILE_SIZE, fim); ti++) {
                    for (int tj = j; tj < min(j + TILE_SIZE, N); tj++) {
                       setValue (resultado, ti, tj, (getValue(matriz1,ti,tj) + getValue (matriz2,ti,tj)));
                    }
                }
            }
    }
...
*/
        ...
        somarMatrizes(matriz1, matriz2, resultado, N, thread_id, num_threads, TILE_SIZE);
